...
@section('content')
    ...
    <p>
        <ul class="list-inline">
            ...
            <li><a href="{{ route('admin.lessons.index') }}?show_deleted=1" style="{{ request('show_deleted') == 1 ? 'font-weight: 700' : '' }}">Удаленные</a></li>
        </ul>
    </p>
...
@section('javascript') 
    <script>
        @can('lesson_delete')
                @if ( request('show_deleted') != 1 )
                    window.route_mass_crud_entries_destroy = '{{ route('admin.lessons.mass_destroy') }}';
                @else
                    window.route_mass_crud_entries_destroy = '{{ route('admin.lessons.mass_permadel') }}';
                @endif
        @endcan
